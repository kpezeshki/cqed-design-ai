...
class coplanar_coupler:
    def __init__(self, s1=None,s2=None,w1=None,w2=None,w3=None,lk=None):
        self.s1 = s1
        self.s2 = s2
        self.w1 = w1
        self.w2 = w2
    ...
    def z_branch_points_notch(self, s1=None,s2=None,w1=None,w2=None,w3=None):
        ...
        z_H = z_G+w1
        z_I = z_H+s1    
        return [np.complex128(x) for x in [z_B, z_C, z_D, z_E, z_F, z_G, z_H, z_I]]
    ...
    def conformal_transform_difference_by_parts(self,z0,z1,branch_point1,branch_point2,branch_points,zero_points):
        ...
        # real part
        real_val, _ = integrate.quad(
            lambda t: f2(t * (z1 - z0) + z0).real,
            0, 1
        )

        # imaginary part
        imag_val, _ = integrate.quad(
            lambda t: f2(t * (z1 - z0) + z0).imag,
            0, 1
        )

        # combine into one complex number
        numerical_part = np.complex128(real_val + 1j * imag_val)
        #numerical_part = np.complex128(*tuple(integrate.quad(lambda t: f2(t*(z1-z0)+z0).real, 0, 1)))+1j*np.complex128(*tuple(integrate.quad(lambda t: f2(t*(z1-z0)+z0).imag, 0, 1)))
    ...
    def coupling_matrices(self,epsilon_eff=None,s1=None,s2=None,w1=None,w2=None,w3=None,mode='notch'):
        ...
        L_l[0,0] = L_l[0,0]+self.lk
        L_l[1,1] = L_l[1,1]+self.lk
        coupling_matrix = L_l/np.sqrt(epsilon_0*mu_0*epsilon_eff)
